...
build-frontend:
	yarn dev-build
build-backend:
	env GOOS=linux go build -o ./dist/logicmonitor-plugin_linux_amd64 ./pkg
build-debug:
	env GOOS=linux go build -gcflags="all=-N -l" -o ./dist/logicmonitor-plugin_linux_amd64 ./pkg

# Build for specific platform
build-backend-windows: extension = .exe
build-backend-darwin-arm64:
	env GOOS=darwin GOARCH=arm64 go build -o ./dist/logicmonitor-plugin_darwin_arm64 ./pkg
build-backend-%:
	$(eval filename = logicmonitor-plugin_$*_amd64$(extension))
...
dist-arm-linux-arm-v6:
	env GOOS=linux GOARCH=arm GOARM=6 go build -ldflags="-s -w" -o ./dist/logicmonitor-plugin_linux_arm ./pkg
dist-arm-linux-arm-v7:
	env GOOS=linux GOARCH=arm GOARM=7 go build -ldflags="-s -w" -o ./dist/logicmonitor-plugin_linux_arm ./pkg
dist-arm-linux-arm64:
	env GOOS=linux GOARCH=arm64 go build -ldflags="-s -w" -o ./dist/logicmonitor-plugin_linux_arm64 ./pkg
dist-arm-darwin-arm64:
	env GOOS=darwin GOARCH=arm64 go build -ldflags="-s -w" -o ./dist/logicmonitor-plugin_darwin_arm64 ./pkg
